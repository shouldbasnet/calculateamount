...
set @v_query=null;
/*-------------------------------------------*/	
set @v_query='update data_final_details_t a, data_final_amount b
set a.data_final_id= b.id
where a.billing_operator=b.billing_operator and a.service_id=b.service_id and a.start_date=b.a.start_date and a.end_date=b.end_date';
prepare stmt from @v_query;
execute stmt;
commit;
deallocate prepare stmt; 
...
select a.operator_code, o.name, sum(callduration), round(sum(calculated_amount)/sum(callduration),2), sum(calculated_amount), serviceid, calculated_at, start_date, end_date
from data_final_details_t a
left join
operators o
on a.operator_code=b.code
group by a.operator_code, o.name, serviceid, calculated_at, start_date, end_date;
commit;

update data_final_details_t a,data_final_amount b
set a.data_final_id=b.id
where a.billing_operator=b.operator_code and a.service_id=b.serviceid and a.start_date=b.start_date and a.end_date=b.end_date;
commit;
*/